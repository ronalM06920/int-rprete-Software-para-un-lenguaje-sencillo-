...
function assign(a,b){
	if(a.variable){
		...
		a.expect("array");
		b.expect("array");
		assert(b.length===a.length,"array de diferente longitud");
		for(var i=0;i<a.value.length;i++){
			assert(a.value[i].variable,"Durante la asignación de la lista de variables, variable esperada pero obtenida "+a.value[i].type+". Perhaps you didn't mean to use =?");
...
function multiply(a,b){
	...
	switch(a.type){
		case "number":
			return new Value("number",a.value*b.value);
		break;case "string":
			assert(b.value>=0,"valor negativo de repetición");
...
function divide(a,b){
	a.expect("number");
	b.expect("number");
	assert(b.value!==0,"dividir por 0");
	return new Value("number",a.value/b.value);
}
